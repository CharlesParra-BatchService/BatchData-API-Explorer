...
import { PROPERTY_TYPE_CATEGORY, PROPERTY_TYPE_DETAIL } from '../constants/propertyClassificationCodes';
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SelectFilterField
              ...
              values={filters.propertyTypeCategory?.inList}
              options={PROPERTY_TYPE_CATEGORY}
              onFieldChange={handleChange}
            />

            <SelectFilterField
              label="Property Type Detail"
              category="propertyTypeDetail"
              values={filters.propertyTypeDetail?.inList}
              options={PROPERTY_TYPE_DETAIL}
              onFieldChange={handleChange}
              searchable={true}
            />
          </div>
          ...
          {Object.keys(filters).length > 0 && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
              <p className="text-sm text-blue-800">
                <strong>Active Property Classification Filters:</strong>
                <span className="block mt-1 space-y-1">
                  {Object.entries(filters).map(([key, value]) => {
                    const label = key === 'propertyTypeCategory' ? 'Property Type Category' : 'Property Type Detail';
                    const parts = [];
...
export default PropertyClassificationFilters;
